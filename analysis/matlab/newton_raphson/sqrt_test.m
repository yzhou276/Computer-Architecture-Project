%% Representative test for integer sqrt using Newton-Raphson
% Saves:
%   1) CSV with results
...
%
% Main output:
%   newton_isqrt_representative_test.csv
...
%% Save CSV
T = table( ...
    x_dec, x_hex, bit_length, newton_isqrt, reference_isqrt, ...
    error_val, iterations, perfect_square, ok);

writetable(T, 'newton_isqrt_representative_test.csv');

%% Save test values
fid = fopen('newton_isqrt_test_values.txt', 'w');
for i = 1:n
    fprintf(fid, '%u\n', x_dec(i));
end
fclose(fid);
...
%% Save summary
fid = fopen('newton_isqrt_test_summary.txt', 'w');
...
function [y, iter] = isqrt_newton_uint32(x)
% Integer square root using Newton-Raphson
% Returns floor(sqrt(x)) for uint32-range input
    ...
    % Final correction to guarantee floor(sqrt(x))
    while yk * yk > x
        yk = yk - 1;
    end
