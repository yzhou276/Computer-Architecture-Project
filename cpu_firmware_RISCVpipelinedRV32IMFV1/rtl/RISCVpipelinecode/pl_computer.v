...
module pl_computer (      // pipelined cpu
  ...
  //  wire          wpcir;           // pipepc and pipeir write enable
    ...
    wire          rv32m;
    wire          z=~|(da^dd);
    // Signed / unsigned less-than comparators computed in ID, parallel to z.
    // Used by BLT in pl_id_cu.
    wire          lt  = ($signed(da) < $signed(dd));
    wire          mdwait;
    ...
    pl_stage_id id_stage (.mrd(mrd),.mm2reg(mm2reg),.mwreg(mwreg),.erd(erd),.em2reg(em2reg),.ewreg(ewreg),.ecancel(ecancel),.mdwait(mdwait),.dpc(dpc),.inst(inst),
                          ...
                          .edata(edata),.is_auipc(is_auipc),.jal(jal),.z(z),.sqrt(sqrt),.start_sqrt(start_sqrt), .lt(lt));                // ID stage
